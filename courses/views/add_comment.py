...
from courses.models import Lesson, Comment, Course
import datetime
import logging
...
def AddCommentView(request, course_slug, lesson_slug):
    ...
    if 'expire_at' in request.session:
        ...
        if expire_at > datetime.datetime.now().timestamp():
            messages.warning(
                request, 'Sorry, You can not comment today. You have to wait 24 hours to comment again.')
