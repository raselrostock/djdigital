...
class CourseAdmin(admin.ModelAdmin):
    ...
    def get_instructors(self, obj):
        return ", ".join([instructor.first_name for instructor in obj.instructors.all()])

    def get_memberships(self, obj):
        return ", ".join([mem_type.membership_type for mem_type in obj.allowed_membership.all()])
    ...
    def export_courses(self, request, queryset):
        ...
        writer.writerow(['Title', 'Description', 'Extra Information',
                         'Start date', 'Liked', 'Allowed Membership', 'Instructors'])
        courses = queryset.values_list(
            'title', 'description', 'extra_info', 'start_date', 'likes__username', 'allowed_membership__membership_type', 'instructors__username')
...
class CommentAdmin(admin.ModelAdmin):
    model = Comment
    list_display = ('author', 'msg', 'comment_at',
                    'lesson', 'approved_comment')
